...
// Response shape mirrors /api/model/data/[name] for each model, grouped under `items`
export async function GET() {
  try {
    // Try upstream first if available
    const remote = await callRemote("/models/data", { method: "GET" });
    if (remote) {
      const data = await remote.json();
      return NextResponse.json(data, { status: remote.status });
    }

    // Local fallback: infer models from public/photos, attach checked models and video counts

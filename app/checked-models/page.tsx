  ...
  return (
    <div className="relative">
      ...
      <section className={`${isOpen ? "md:ml-64" : "md:ml-0"} ml-0 py-4 md:py-8 transition-[margin] duration-300 px-4 md:px-6`}>
        ...
        {error && (
          <div role="alert" className="mb-4 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-200">Nu s-au putut înc��rca modelele.</div>
        )}
...
function EditModal({ mode, item, onClose, onSaved }: { mode: "create" | "edit"; item?: CheckedModel; onClose: () => void; onSaved: () => void }) {
  ...
  const { addToast } = useToast();
  type ModelOption = { id: number; name: string; imageUrl?: string; isOnline?: boolean };
                  ...
                  {!modelsLoading && !modelsError && (filteredModels.length === 0 ? (
                    ...
                    filteredModels.map((m) => (
                      <button
                        ...
                        key={m.id}
                        onClick={() => pickModel(m)}
                        className="w-full text-left px-3 py-2 hover:bg-white/10 focus:bg-white/10 flex items-center gap-3"
                        title={`#${m.id}`}
                      >
                        <div className="w-8 h-8 rounded overflow-hidden bg-white/10 flex items-center justify-center">
                          {m.imageUrl ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={m.imageUrl} alt={m.name} className="w-full h-full object-cover" />
                          ) : (
                            <div className="text-xs text-white/60">No img</div>
                          )}
                        </div>
                        ...
                        {m.isOnline ? (
                          <span className="ml-auto inline-flex items-center gap-1 text-xs text-green-300"><span className="w-2 h-2 rounded-full bg-green-400"></span>Online</span>
                        ) : null}
                      </button>
                    ))
                  ))}
